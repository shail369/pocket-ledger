...
function AccountDetail() {
  ...
  const history = data.transactions
    .filter((tx) => tx.account_id === accountId || tx.transfer_account_id === accountId)
    .slice(0, 40);
  ...
  return (
    <div className="space-y-4">
      <header className="flex items-center gap-2">
        <Link to="/accounts" className="grid size-9 place-items-center rounded-full bg-secondary">
          <ChevronLeft className="size-5" />
        </Link>
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-lg font-extrabold">{account.name}</h1>
          <p className="text-[11px] capitalize text-muted-foreground">
            {account.type} · {account.is_active ? "Active" : "Archived"}
          </p>
        </div>
        <span className="grid size-10 place-items-center rounded-2xl text-white" style={{ backgroundColor: account.color }}>
          <AppIcon name={account.icon} className="size-4" />
        </span>
      </header>
      ...
      <div className="rounded-3xl bg-primary p-4 text-primary-foreground">
        ...
        <p className="mt-1 text-[11px] opacity-80">
          Opening {formatMoney(Number(account.opening_balance), account.currency)}
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <StatCard label="Savings" value={formatMoney(t.savings, currency, true)} tone="primary" />
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <Button variant="secondary" className="h-11 rounded-xl" onClick={() => setEdit(true)}>
        ...
        <Button
          ...
          onClick={async () => {
            await upsert.mutateAsync({ id: account.id, is_active: !account.is_active });
            toast.success(account.is_active ? "Account archived" : "Account restored");
          }}
        >
          <Archive className="size-4" /> {account.is_active ? "Archive" : "Restore"}
        </Button>
        <Button
          variant="destructive"
          className="h-11 rounded-xl"
