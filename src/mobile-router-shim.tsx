...
export function useNavigate() {
  ...
  if (!context) throw new Error("useNavigate must be used inside the mobile router");
...
export function useRouterState<T>({ select }: { select: (state: { location: { pathname: string } }) => T }) {
  const context = useContext(MobileRouterContext);
  if (!context) throw new Error("useRouterState must be used inside the mobile router");
...
export function createFileRoute(_path: string) {
  return (options: RouteOptions) => ({
    ...options,
    useParams: () => {
      const { pathname } = useContext(MobileRouterContext) ?? { pathname: "/" };
