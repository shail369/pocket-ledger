...
--
-- This migration is intentionally idempotent so it can safely establish the
-- current schema on a fresh Supabase project and remain safe to re-run while
-- the project is being restored.
--
-- It creates schema only plus the default category seeding function.
-- It never creates demo accounts, transactions, budgets, or recurring data.
--
-- Future schema changes should be added as new migrations after this file.
...
CREATE POLICY "own saving goals"
...
--
-- A contribution reserves money from an account.
-- It is NOT a transaction and does NOT create a transaction row.
-- ============================================================
...
CREATE INDEX IF NOT EXISTS saving_goal_contributions_user_idx
  ON public.saving_goal_contributions(user_id);

CREATE INDEX IF NOT EXISTS saving_goal_contributions_goal_idx
  ON public.saving_goal_contributions(goal_id);

CREATE INDEX IF NOT EXISTS saving_goal_contributions_account_idx
  ON public.saving_goal_contributions(account_id);
...
DROP POLICY IF EXISTS "own saving goal contributions"
  ON public.saving_goal_contributions;

CREATE POLICY "own saving goal contributions"
  ON public.saving_goal_contributions
  FOR ALL TO authenticated
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);
...
DROP TRIGGER IF EXISTS saving_goals_updated_at
  ON public.saving_goals;

CREATE TRIGGER saving_goals_updated_at
BEFORE UPDATE ON public.saving_goals
FOR EACH ROW
EXECUTE FUNCTION public.touch_saving_goal_updated_at();


-- ============================================================
-- SAVING GOAL CONTRIBUTION VALIDATION
-- ============================================================
...
BEGIN
  SELECT
    a.opening_balance
    + COALESCE(SUM(CASE
        WHEN t.type = 'income' AND t.account_id = NEW.account_id
        THEN t.amount ELSE 0 END), 0)
    - COALESCE(SUM(CASE
        WHEN t.type = 'expense' AND t.account_id = NEW.account_id
        THEN t.amount ELSE 0 END), 0)
    - COALESCE(SUM(CASE
        WHEN t.type = 'transfer' AND t.account_id = NEW.account_id
        THEN t.amount ELSE 0 END), 0)
    + COALESCE(SUM(CASE
        WHEN t.type = 'transfer' AND t.transfer_account_id = NEW.account_id
        THEN t.amount ELSE 0 END), 0)
...
DROP TRIGGER IF EXISTS validate_saving_goal_contribution
  ON public.saving_goal_contributions;

CREATE TRIGGER validate_saving_goal_contribution
BEFORE INSERT OR UPDATE ON public.saving_goal_contributions
FOR EACH ROW
EXECUTE FUNCTION public.validate_saving_goal_contribution();


-- ============================================================
-- DEFAULT CATEGORY SEED
--
-- This is the category section from Pocket Ledger's original
-- seed_demo_data function. Nothing else from that demo seed is
-- retained here.
--
-- Running this repeatedly is safe: existing categories are kept
-- and only missing category rows are inserted.
-- ============================================================
...
BEGIN
  ...
  -- Expense categories: copied from the original seed data.
  FOR category IN
    SELECT * FROM (VALUES
  ...
  LOOP
    ...
    FOREACH subcategory IN ARRAY category.subcategories
    LOOP
      INSERT INTO public.categories(user_id, name, parent_id, icon, kind)
      SELECT uid, subcategory, parent_id, category.icon, 'expense'
      WHERE NOT EXISTS (
        SELECT 1
  ...
  -- Income categories: copied from the original seed data.
  INSERT INTO public.categories(user_id, name, icon, kind)
  SELECT uid, category.name, category.icon, 'income'
  FROM (VALUES
    ('Salary', 'briefcase'),
    ('Freelance', 'laptop'),
    ('Interest', 'piggy-bank'),
