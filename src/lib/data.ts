...
async function fetchAppData(): Promise<AppData> {
  ...
  const [savingGoals, savingGoalContributions, assetsResult, liabilitiesResult] = await Promise.all([
    ...
    supabase.from("assets" as never).select("*").order("created_at"), supabase.from("liabilities" as never).select("*").order("created_at"),
  ]);
  return {
    accounts: (accounts.data ?? []) as unknown as Account[], categories: (categories.data ?? []) as unknown as Category[], transactions, budgets: (budgets.data ?? []) as unknown as Budget[], recurring: (recurring.data ?? []) as unknown as Recurring[],
    savingGoals: savingGoals.error ? [] : ((savingGoals.data ?? []) as unknown as SavingGoal[]), savingGoalContributions: savingGoalContributions.error ? [] : ((savingGoalContributions.data ?? []) as unknown as SavingGoalContribution[]),
    assets: assetsResult.error ? [] : ((assetsResult.data ?? []) as unknown as Asset[]), liabilities: liabilitiesResult.error ? [] : ((liabilitiesResult.data ?? []) as unknown as Liability[]),
  };
}
...
type Table = "accounts" | "categories" | "transactions" | "budgets" | "recurring_transactions" | "saving_goals" | "saving_goal_contributions" | "assets" | "liabilities";
...
export function useUpsert(table: Table) {
  ...
  return useMutation({ mutationFn: async (row: Record<string, unknown>) => { const { data: userRes } = await supabase.auth.getUser(); const payload = { ...row, user_id: userRes.user?.id }; const rowId = row["id"] as string | undefined; const clientTable = (table === "assets" || table === "liabilities") ? (table as never) : table; const result = rowId ? await supabase.from(clientTable).update(row as never).eq("id", rowId).select("*").single() : await supabase.from(clientTable).insert(payload as never).select("*").single(); if (result.error) throw result.error; return result.data as never; }, onSuccess: (saved) => { queryClient.setQueryData<AppData>(DATA_KEY, (current) => { if (!current) return current; const key = dataKey(table); const rows = current[key] as unknown as Record<string, unknown>[]; const id = String((saved as Record<string, unknown>).id ?? ""); const index = rows.findIndex((r) => String(r.id) === id); const nextRows = index >= 0 ? rows.map((r, i) => (i === index ? saved : r)) : [saved, ...rows]; return { ...current, [key]: nextRows } as AppData; }); } });
}

export function useRemove(table: Table) {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: async (id: string) => { const clientTable = (table === "assets" || table === "liabilities") ? (table as never) : table; if (table === "saving_goals") { const { error: contributionError } = await supabase.from("saving_goal_contributions").delete().eq("goal_id", id); if (contributionError) throw contributionError; } const { error } = await supabase.from(clientTable).delete().eq("id", id); if (error) throw error; return id; }, onSuccess: (id) => { queryClient.setQueryData<AppData>(DATA_KEY, (current) => { if (!current) return current; const next = { ...current }; const key = dataKey(table); const rows = next[key] as unknown as { id: string }[]; next[key] = rows.filter((row) => row.id !== id) as never; if (table === "saving_goals") next.savingGoalContributions = next.savingGoalContributions.filter((c) => c.goal_id !== id); return next; }); } });
}
