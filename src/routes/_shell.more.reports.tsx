...
function ReportsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === "monthly" && <>
        <Section title="Income vs expenses" subtitle={`${scopeLabel} · Last 12 months`}><MonthlyComparisonChart data={months} /></Section>
      ...
      {tab === "yearly" && <>
        ...
        <Section title={`Year ${thisYear}`} subtitle={`${yearTx.length} transactions · ${scopeLabel}`}><ul className="divide-y divide-border/60">{months.slice(-12).map((m) => <li key={m.key} className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 py-2"><span className="truncate text-sm">{format(parseISO(`${m.key}-01`), "MMM yyyy")}</span><span className={`tabular text-sm font-bold ${m.savings >= 0 ? "text-income" : "text-expense"}`}>{m.savings >= 0 ? "+" : "−"}{formatMoney(Math.abs(m.savings), currency)}</span></li>)}</ul></Section>
      </>}

      {tab === "category" && <Section title="Category report" subtitle={`Year ${thisYear} · ${scopeLabel}`}>{nodes.length ? <ul className="divide-y divide-border/60">{nodes.map((n) => <li key={n.id} className="py-2.5"><div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-secondary"><AppIcon name={n.icon} className="size-4" /></span><span className="min-w-0 truncate text-sm font-semibold">{n.name}</span><span className="text-right"><span className="tabular block text-sm font-bold">{formatMoney(n.amount, currency)}</span><span className="block text-[11px] text-muted-foreground">{percent(n.percent)}</span></span></div></li>)}</ul> : <EmptyState text="No spending recorded this year." />}</Section>}
    </div>
  );
}