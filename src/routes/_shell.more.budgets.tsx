  ...
  return (
    <div className="space-y-4">
      <header className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2">
        ...
        <button onClick={() => { setEditing(null); setOpen(true); }} className="grid size-10 place-items-center rounded-full bg-primary text-primary-foreground" aria-label="Add budget"><Plus className="size-5" /></button>
      </header>
        ...
        <Section key={`${b.categoryName}-${b.rangeLabel}-${b.budget.id}`}>
          <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3">
            ...
            <button className="min-w-0 text-left" onClick={() => { if (accountId === "all") return; setEditing(b.budget); setOpen(true); }}>
              ...
              <p className="text-[11px] capitalize text-muted-foreground">{b.budget.period} · {b.rangeLabel} · {accountId === "all" ? "Combined" : "Selected account"}</p>
            </button>
...
function BudgetForm({ open, onOpenChange, existing, selectedAccountId }: { open: boolean; onOpenChange: (v: boolean) => void; existing?: Budget | null; selectedAccountId: string }) {
  ...
  const submit = async () => {
    if (!account) { toast.error("Select an account for this budget"); return; }
    if (!amount || Number(amount) <= 0) { toast.error("Enter a budget amount"); return; }
    try {
      await upsert.mutateAsync({ ...(existing ? { id: existing.id } : {}), category_id: category === OVERALL ? null : category, account_id: account, amount: Number(amount).toFixed(2), period });
      toast.success(existing ? "Budget updated" : "Budget created"); onOpenChange(false);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[90vh] overflow-y-auto rounded-t-3xl">
        <SheetHeader><SheetTitle>{existing ? "Edit budget" : "New budget"}</SheetTitle></SheetHeader>
        <div className="space-y-4 px-4 pb-8">
          ...
          <div className="space-y-1.5"><Label className="text-xs">Category</Label><Select value={category} onValueChange={setCategory}><SelectTrigger className="h-12 w-full rounded-xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={OVERALL}>Overall budget</SelectItem>{parents.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5"><Label className="text-xs">Amount</Label><Input inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} className="h-12 rounded-xl" /></div>
            <div className="space-y-1.5"><Label className="text-xs">Period</Label><Select value={period} onValueChange={setPeriod}><SelectTrigger className="h-12 w-full rounded-xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="weekly">Weekly</SelectItem><SelectItem value="monthly">Monthly</SelectItem></SelectContent></Select></div>
          </div>
          <Button className="h-12 w-full rounded-xl" onClick={submit} disabled={upsert.isPending}>{existing ? "Save budget" : "Create budget"}</Button>
        </div>
      </SheetContent>
    </Sheet>
  );
