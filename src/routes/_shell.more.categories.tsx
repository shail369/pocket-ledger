...
export const Route = createFileRoute("/_shell/more/categories")({
  head: () => ({ meta: [{ title: "Categories — Spendify" }, { name: "description", content: "Manage expense and income categories and subcategories." }] }),
...
function CategoriesPage() {
  ...
  const deleteCategory = async (category: Category) => {
    ...
    const parts = [`Delete ${category.name}${descendants.length ? ` and its ${descendants.length} subcategor${descendants.length === 1 ? "y" : "ies"}` : ""}?`, transactionCount ? `${transactionCount} transaction${transactionCount === 1 ? "" : "s"} will become uncategorized.` : "", recurringCount ? `${recurringCount} recurring transaction${recurringCount === 1 ? "" : "s"} will become uncategorized.` : "", budgetCount ? `${budgetCount} related budget${budgetCount === 1 ? "" : "s"} will be deleted.` : ""].filter(Boolean);
    if (!confirm(parts.join("\n\n"))) return;
    try { await removeCategory.mutateAsync(category.id); toast.success("Category deleted"); } catch (e) { toast.error((e as Error).message); }
  };

  return <div className="space-y-4 pb-2">
    <header className="flex items-center gap-2"><Link to="/more" className="grid size-9 place-items-center rounded-full bg-secondary"><ChevronLeft className="size-5" /></Link><div className="min-w-0 flex-1"><h1 className="truncate text-lg font-extrabold">Categories</h1><p className="truncate text-[11px] text-muted-foreground">Organize your transactions</p></div><Button onClick={openNewCategory} className="h-9 rounded-xl px-3 text-xs"><Plus className="size-3.5" /> Add</Button></header>
    {parents.length ? <div className="space-y-3">{parents.map((parent) => { const children = data.categories.filter((c) => c.parent_id === parent.id); return <Section key={parent.id} className="overflow-hidden p-0"><div className="flex items-center gap-3 border-b border-border/60 px-3.5 py-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-secondary"><AppIcon name={parent.icon} className="size-4" /></span><button className="min-w-0 flex-1 text-left" onClick={() => openEditCategory(parent)}><span className="block truncate text-sm font-semibold">{parent.name}</span><span className="text-[10px] font-medium capitalize text-muted-foreground">{parent.kind} · {children.length} subcategor{children.length === 1 ? "y" : "ies"}</span></button><div className="flex shrink-0 items-center gap-1.5"><button onClick={() => openEditCategory(parent)} className="grid size-8 place-items-center rounded-lg bg-secondary text-muted-foreground" aria-label={`Edit ${parent.name}`}><Pencil className="size-3.5" /></button><button onClick={() => void deleteCategory(parent)} className="grid size-8 place-items-center rounded-lg bg-secondary text-expense" aria-label={`Delete ${parent.name}`}><Trash2 className="size-3.5" /></button></div></div>{children.length ? <div className="divide-y divide-border/50">{children.map((child) => <div key={child.id} className="flex items-center gap-3 px-3.5 py-2.5"><span className="ml-1 grid size-8 shrink-0 place-items-center rounded-lg bg-secondary/70 text-muted-foreground"><AppIcon name={child.icon} className="size-3.5" /></span><button onClick={() => openEditCategory(child)} className="min-w-0 flex-1 text-left"><span className="block truncate text-xs font-medium">{child.name}</span></button><div className="flex shrink-0 items-center gap-1"><button onClick={() => openEditCategory(child)} className="grid size-7 place-items-center rounded-md text-muted-foreground hover:bg-secondary" aria-label={`Edit ${child.name}`}><Pencil className="size-3" /></button><button onClick={() => void deleteCategory(child)} className="grid size-7 place-items-center rounded-md text-expense hover:bg-expense/10" aria-label={`Delete ${child.name}`}><Trash2 className="size-3" /></button></div></div>)}</div> : <p className="px-4 py-3 text-[11px] text-muted-foreground">No subcategories. Use Edit to create one.</p>}</Section>; })}</div> : <EmptyState text="No categories yet. Add your first category to get started." />}
...
function CategoryForm({ open, onOpenChange, existing }: { open: boolean; onOpenChange: (v: boolean) => void; existing?: Category | null }) {
  ...
  const submit = async () => {
    ...
    try { await upsert.mutateAsync({ ...(existing ? { id: existing.id } : {}), name: name.trim(), kind, parent_id: parent === NONE ? null : parent, icon }); toast.success(isEditing ? "Category updated" : "Category added"); onOpenChange(false); }
  ...
  const addSubcategory = async () => {
    ...
    try { await upsert.mutateAsync({ name: value, kind: existing.kind, parent_id: existing.id, icon: "tag" }); setNewSubcategory(""); setAddingSubcategory(false); toast.success("Subcategory added"); }
    catch (e) { toast.error((e as Error).message); }
  };

  return <Sheet open={open} onOpenChange={onOpenChange}>
    <SheetContent side="bottom" className="max-h-[78dvh] overflow-y-auto rounded-t-3xl px-0">
      <SheetHeader className="px-5 pb-2 pt-1"><SheetTitle>{isEditing ? "Edit category" : "New category"}</SheetTitle></SheetHeader>
      <div className="space-y-4 px-5 pb-5">
        {!isEditing && <div className="inline-flex rounded-xl bg-secondary p-1"><button type="button" onClick={() => { setKind("expense"); setParent(NONE); }} className={`h-9 rounded-lg px-5 text-xs font-semibold capitalize ${kind === "expense" ? "bg-card shadow-sm" : "text-muted-foreground"}`}>Expense</button><button type="button" onClick={() => { setKind("income"); setParent(NONE); }} className={`h-9 rounded-lg px-5 text-xs font-semibold capitalize ${kind === "income" ? "bg-card shadow-sm" : "text-muted-foreground"}`}>Income</button></div>}
        <div className="space-y-1.5"><Label className="text-xs">Name</Label><Input value={name} onChange={(e) => setName(e.target.value)} className="h-11 rounded-xl" /></div>
        {(!isEditing || isSubcategory) && <div className="space-y-1.5"><Label className="text-xs">Parent category</Label><Select value={parent} onValueChange={setParent}><SelectTrigger className="h-11 w-full rounded-xl"><SelectValue /></SelectTrigger><SelectContent><SelectItem value={NONE}>None (top level)</SelectItem>{parents.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></div>}
        ...
        <div className="space-y-1.5"><Label className="text-xs">Icon</Label><div className="no-scrollbar flex gap-2 overflow-x-auto pb-1">{ICON_OPTIONS.map((i) => <button type="button" key={i} onClick={() => setIcon(i)} className={`grid size-10 shrink-0 place-items-center rounded-xl ${icon === i ? "bg-primary text-primary-foreground" : "bg-secondary"}`}><AppIcon name={i} className="size-4" /></button>)}</div></div>
        <Button className="h-11 w-full rounded-xl" onClick={submit} disabled={upsert.isPending}>{isEditing ? "Save changes" : "Add category"}</Button>
      </div>
    </SheetContent>
  </Sheet>;
}
