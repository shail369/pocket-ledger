          ...
          <Section key={`${b.budget.id}-${b.budget.category_id ?? "overall"}-${b.budget.period}`}>
            <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3">
              ...
              <button
                ...
                className="grid size-9 place-items-center rounded-xl bg-secondary text-expense"
            ...
            <Progress
              ...
              className={`mt-3 h-2 ${b.state === "over" ? "[&>div]:bg-expense" : b.state === "warning" ? "[&>div]:bg-warning" : ""}`}
            />
            <div className="mt-1.5 flex items-center justify-between text-[11px] text-muted-foreground">
              <span className="tabular">
                {formatMoney(b.spent, currency)} of {formatMoney(Number(b.budget.amount), currency)}
              </span>
              <span className={b.remaining < 0 ? "font-bold text-expense" : ""}>
                {b.remaining < 0
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="max-h-[90vh] overflow-y-auto rounded-t-3xl">
        <SheetHeader>
          <SheetTitle>{existing ? "Edit budget" : "New budget"}</SheetTitle>
        </SheetHeader>
