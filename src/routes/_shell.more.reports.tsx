...
function ReportsPage() {
  ...
  const budgetMonths = useMemo(() => monthlyBudgetReport(data.budgets, data.transactions, data.categories, accountId, 6), [data.budgets, data.transactions, data.categories, accountId]);
  const budgetByMonth = useMemo(() => new Map(budgetMonths.map((m) => [m.key, m])), [budgetMonths]);
      ...
      <Section title="Monthly summary">
        <ul className="divide-y divide-border/60">
          {[...months].reverse().map((m) => {
            const net = m.income - m.expense; const budget = budgetByMonth.get(m.key); const open = expandedMonth === m.key;
            return <li key={m.key} className="py-3">
              <button className="grid w-full grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 text-left" onClick={() => budget && setExpandedMonth(open ? null : m.key)}>
                <span className="min-w-0"><span className="block truncate text-sm font-semibold">{format(parseISO(`${m.key}-01`), "MMMM yyyy")}</span>{budget && <span className={`mt-1 inline-flex rounded-full px-1.5 py-0.5 text-[9px] font-bold ${budget.overall.state === "over" ? "bg-expense-soft text-expense" : budget.overall.state === "warning" ? "bg-warning/15 text-warning" : "bg-income-soft text-income"}`}>{budget.overall.state === "over" ? "Over budget" : budget.overall.state === "warning" ? "Near limit" : "Within budget"}</span>}</span>
                <span className={`tabular text-sm font-semibold ${net >= 0 ? "text-income" : "text-expense"}`}>{net >= 0 ? "+" : "−"}{formatMoney(Math.abs(net), currency)}</span>
                {budget ? (open ? <ChevronUp className="size-4 text-muted-foreground" /> : <ChevronDown className="size-4 text-muted-foreground" />) : <span className="size-4" />}
              </button>
              {budget && <div className="mt-2"><div className="flex items-center justify-between text-[10px] text-muted-foreground"><span>Budget {formatMoney(budget.overall.budget, currency)}</span><span>{formatMoney(budget.overall.spent, currency)} spent · {budget.overall.percent.toFixed(0)}%</span></div><div className="mt-1 h-1.5 overflow-hidden rounded-full bg-secondary"><div className={`h-full ${budget.overall.state === "over" ? "bg-expense" : budget.overall.state === "warning" ? "bg-warning" : "bg-income"}`} style={{ width: `${Math.min(100, budget.overall.percent)}%` }} /></div></div>}
              {budget && open && <div className="mt-3 rounded-2xl bg-secondary/50 p-3"><div className="mb-2 flex items-center justify-between"><p className="text-[11px] font-bold uppercase tracking-wide text-muted-foreground">Budget details</p><p className="text-[10px] text-muted-foreground">{budget.overall.remaining < 0 ? `${formatMoney(Math.abs(budget.overall.remaining), currency)} over` : `${formatMoney(budget.overall.remaining, currency)} left`}</p></div>{budget.categories.length ? <div className="space-y-2">{budget.categories.map((item) => <div key={`${item.budget.id}-${m.key}`} className="flex items-center gap-2 rounded-xl bg-card p-2.5"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-secondary"><AppIcon name={item.icon} className="size-3.5" /></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">{item.categoryName}</p><p className="text-[10px] text-muted-foreground">{formatMoney(item.spent, currency)} of {formatMoney(Number(item.budget.amount), currency)}</p></div><span className={`text-[11px] font-bold ${item.state === "over" ? "text-expense" : item.state === "warning" ? "text-warning" : "text-income"}`}>{item.percent.toFixed(0)}%</span></div>)}</div> : <p className="text-xs text-muted-foreground">No individual category budgets.</p>}</div>}
            </li>;
          })}
        </ul>
      </Section>
