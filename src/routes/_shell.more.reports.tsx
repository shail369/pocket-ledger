...
import { accountBalance, categoryBreakdown, monthlySeries, scopeTransactions, totals } from "@/lib/finance";
  ...
  return (
    <div className="space-y-4">
      <header className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2">
        ...
        <Button variant="secondary" className="h-10 rounded-xl" onClick={exportCsv}>
      ...
      <div className="no-scrollbar flex gap-2 overflow-x-auto">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`h-9 shrink-0 rounded-full px-4 text-xs font-semibold capitalize ${
      ...
      {tab === "yearly" && (
        <>
          <div className="grid grid-cols-3 gap-2">
            <StatCard label="Income" value={formatMoney(yearTotals.income, currency, true)} tone="income" />
      ...
      {tab === "category" && (
        <Section title="Category report" subtitle={`Year ${thisYear}`}>
          {nodes.length ? (
                ...
                <li key={n.id} className="py-2.5">
                  <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3">
                    ...
                    <span className="text-right">
                      ...
                      <span className="block text-[11px] text-muted-foreground">{percent(n.percent)}</span>
                    </span>
                  </div>
                </li>
              ))}
            ...
            <EmptyState text="No spending recorded this year." />
          )}
        </Section>
      )}
      ...
      {tab === "account" && (
        <Section title="Account report">
          <ul className="divide-y divide-border/60">
            {data.accounts.map((a) => {
              const tx = data.transactions.filter((t) => t.account_id === a.id || t.transfer_account_id === a.id);
              const t = totals(tx);
              return (
