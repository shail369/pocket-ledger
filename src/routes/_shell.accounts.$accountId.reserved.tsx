...
import { reservedForAccount, reservedByGoal } from "@/lib/saving-goal-reservations";
...
function AccountDetail() {
  ...
  const goalReservations = data.savingGoals.map((goal) => ({ goal, amount: data.savingGoalContributions.filter((c) => c.account_id === accountId && c.goal_id === goal.id).reduce((sum, c) => sum + Number(c.amount), 0) })).filter(({ amount }) => amount > 0);
  return <div className="space-y-4"><header className="flex items-center gap-2"><Link to="/accounts" className="grid size-9 place-items-center rounded-full bg-secondary"><ChevronLeft className="size-5" /></Link><div className="min-w-0 flex-1"><h1 className="truncate text-lg font-extrabold">{account.name}</h1><p className="text-[11px] capitalize text-muted-foreground">{account.type} · {account.is_active ? "Active" : "Archived"}</p></div><span className="grid size-10 place-items-center rounded-2xl text-white" style={{ backgroundColor: account.color }}><AppIcon name={account.icon} className="size-4" /></span></header>
    ...
    {goalReservations.length > 0 && <Section title="Reserved in saving goals"><div className="divide-y divide-border/60">{goalReservations.map(({ goal, amount }) => <div key={goal.id} className="flex items-center gap-3 py-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl" style={{ backgroundColor: `${goal.color}18`, color: goal.color }}><AppIcon name={goal.icon} className="size-4" /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{goal.name}</p><p className="text-[11px] text-muted-foreground">Reserved from this account</p></div><span className="tabular text-sm font-bold">{formatMoney(amount, account.currency)}</span></div>)}</div></Section>}
